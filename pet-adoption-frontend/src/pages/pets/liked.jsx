...
export default function LikedPetsPage() {
    ...
    const { getLikedAnimals } = animalService();
    ...
    React.useEffect(() => {
        async function load() {
            ...
            await getLikedAnimals(currentUserId, quantityPerPage, [])
                .then((result) => {
                    if (result != null) {
                        if (result.length < 1) {
                            setHasMore(false);
    ...
    const fetchMoreData = async () => {
        ...
        await getLikedAnimals(currentUserId, quantityPerPage, previousIds)
            .then((result) => {
                if (result != null) {
                    if (result.length < 1) {
                        setHasMore(false);
                    ...
                    <Card sx={{ width: "80%", position: "relative" }} elevation={4}>
                        <CardContent>
                            ...
                            <Typography variant="body1" align="center" color="text.secondary">
                                Below is a list of all animals you've liked. Click on one for more info, or to inquire about adopting it!
                            </Typography>
